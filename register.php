...
if($_POST['BtnSubmit']=='Submit')
{
	...
	$rs_duplicate = mysql_query("select count(*) as total from member_master where m_emailid='$um_emailid'") or die(mysql_error());
	list($total) = mysql_fetch_row($rs_duplicate);
	
	if ($total > 0)
	{
		$err = urlencode("ERROR: This login email already exists. Please try again with different email id.");
	...
	//get max id
	$qr_id = mysql_query("select IFNULL(max(m_id),0) + 1 from member_master") or die(mysql_error());
	list($um_id) = mysql_fetch_row($qr_id);
	
	//insert
	$sql_insert = "INSERT into member_master

		    VALUES
		    ($um_id,'$um_emailid','$um_name','$um_password',null,null,null,null,now(),1,null,null,null,null,'$usertype')";
...
//echo $sql_insert;
	...
	$sql="SELECT * FROM survey_admin_users WHERE sau_name='$myusername' and sau_passwd='$mypassword'";
