...
<?php
	...
	if($_POST['BtnSubmit']=='Submit')
	{
		...
		if ($mode=='edit')
		{
			...
			{
				$cond="and sc_create_by=".$_SESSION['userid'];
			}
		...
		if ($mode=='add')
		{
			...
			$result=mysql_query("select max(ec_id)+1 as m from  exam_category" );
			
			$id_=mysql_result($result,0,"m"); 
			
			$uid=$_SESSION['userid'];
			
		    $sql="insert into  exam_category values($id_,'$txt_name', '$txt_desc',$uid,null,$chk_dis)"; 
